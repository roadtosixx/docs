...
## RoadPhone Fold compatibility

RoadPhone Fold is included with Pro and uses the same custom-app registration and `window.roadphone` API. The current custom-app route stays in a centered phone-width column when Fold is unfolded. There is no public SDK method or Lua export for directly setting Fold state. See [Fold integration](/roadphonepro/fold#integration-and-exports).
...
- **Metadata mode** (`Config.UseMetadata = true`): the list of installed apps
  is stored on the **phone item's metadata** (`installed_apps`, up to 100 apps
...
<Tip>
  **[Custom App RPC](/roadphonepro/custom-app-rpc)** covers this in depth: both
  transports and when each one fails, timeouts, the dispatcher pattern with a
  reply guard, and how to keep the UI in sync without a push channel.
</Tip>
