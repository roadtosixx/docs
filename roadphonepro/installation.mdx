...
<Info>
  **RoadPhone Fold is included with RoadPhone Pro.** Install the same `roadphone` resource, then follow the [Fold setup](/roadphonepro/fold) for inventory items, controls and 3D props. For existing servers, see [Pro & Fold Updates](/roadphonepro/updates).
</Info>
...
<Tabs>
    ...
    <Tab title="jaksam_inventory">
        RoadPhone registers configured phone items for use. Jaksam supports both the regular `phone` and the separate `phone_fold` item. Create or enable `phone_fold` in the inventory and list it in `Config.FoldableItems`; see [Fold setup](/roadphonepro/fold).
    </Tab>
</Tabs>
