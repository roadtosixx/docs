...
| `jaksam` | full | Works out of the box; the item is always `phone` |
...
The item stores device data and pointers to signed-in accounts; account content remains in the database. Fields are created on demand. The optional `phone_model` below illustrates a Jaksam Fold item; omit it on regular Pro phones. See [Accounts & Sessions](/roadphonepro/accounts) and [Fold persistence](/roadphonepro/fold#fold-and-unfold).
...
```lua
{
    ...
    phone_model          = "ultra",       -- Fold selector on Jaksam; omit for regular phones
...
<Warning>
  `phone_account` and `phone_twitter_account` are **dead fields**. Accounts moved to server-authoritative sessions — the fields are only read once, migrated into a session and then cleared. Never write them.
</Warning>
