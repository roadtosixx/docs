...
RoadPhone can run without any framework. If neither `es_extended`, `qb-core` nor `qbx_core` is started, the Bridge automatically falls back to **standalone mode** — no configuration flag needed.
