...
<AccordionGroup>
  ...
  <Accordion title="A widget will not stick to a surface">
    Three reasons, in order of likelihood: the surface is tilted more than
    `Config.Widgets.maxSurfaceTilt` and counts as floor or ceiling; you are
    further away than `Config.Widgets.reach` (6 m); or you already have
    `Config.Widgets.max` widgets up (12).
  </Accordion>

  <Accordion title="My widgets disappeared">
    Look at the distance first — beyond `Config.Widgets.renderDistance` (20 m)
    a widget is not drawn, but it is still saved.

    They also step aside once `Config.Widgets.hideAtPanels` app windows are
    awake in view at the same time (4 by default). Close a window and they come
...
<AccordionGroup>
  <Accordion title="The frame rate drops with a lot of windows open">
    `Config.MaxPanels` (6) caps app windows and `Config.Widgets.max` (12) caps
