...
{product} ships with a GIF picker in the Messages app (and every other app that supports media attachments), powered by [Klipy](https://klipy.com). All API requests run **server-side** — the API key never reaches the client.
...
<Steps>
    ...
    <Step title="Restart the resource">
        Restart {product} (`ensure roadphone`). The GIF button appears in the Messages chat input automatically.
    </Step>
</Steps>
