...
The Admin Menu is an in-game panel that lets you edit {product}'s configuration files (`config.lua`, `config.json`, addon configs, and locales) directly from inside FiveM — no manual file editing or FTP uploads required.

<Frame>
  <img src="https://r2.fivemanage.com/72cdX4S6uW7YY845UQgsA/iztjjzttzjage.png" alt="Admin Menu" />
</Frame>
...
<Warning>
  This permission gives full read/write access to {product}'s config files on the server filesystem. Only grant it to trusted staff.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Addons" icon="puzzle">
    Lists every addon under `lua-code/addons/*/config.lua` {addons}. Each addon's config is loaded into a separate sub-section so you can configure them independently without leaving the panel.
  </Accordion>
